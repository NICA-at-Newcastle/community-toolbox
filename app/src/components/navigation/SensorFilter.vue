<template lang="pug">
  #sensor-filter(v-bind:class="{ categories: categoriesVisible, search: categories.length > 0 ? searchVisible : true, searchOnly: categories.length === 0 }")
    .row
      .content-block.pull-up.white-block

        .sort-wrapper(v-show="categories.length > 0")
          ul#sort-tabs
            li.type-dropdown(@click="toggleCategories()" v-bind:class="{ 'has-categories': categories.length > 0 }")
        ...
        sensor-type-filter(v-bind:categories.sync="categories" v-show="categories.length > 0")
...
<script>
import SensorTypeFilter from "@/components/filtering/SensorTypeFilter";
...
export default {
  ...
  watch: {
    ...
    currentType(nV) {
      ...
      // }
      this.$emit("update:currentType", nV);
      this.$emit("reload");
    }
  },
  computed: {
    currentType() {
      // Attempt to find a type from the current URL param
      const type = _find(this.categories, {
  ...
  methods: {
    ...
    toggleSearch(to) {
      ...
      this.categoriesVisible = this.searchVisible
        ? false
    ...
    toggleSortType() {
      ...
      this.sortTypeIndex =
        this.sortTypeIndex > this.sortTypes.length - 1 ? 0 : this.sortTypeIndex;
